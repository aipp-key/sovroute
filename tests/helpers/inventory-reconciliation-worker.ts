...
export interface ReconciliationWorkerInitMessage {
  type: 'INIT';
  dbPath: string;
  tokenAddress: string;
  executionId: string;
  ...
  hashLock?: string;
  mode: 'direct' | 'reconciled' | 'coordinator';
}

export interface ReconciliationWorkerStartMessage {
  type: 'START';
}

let config: ReconciliationWorkerInitMessage | null = null;
let persistence: SqlitePersistence | null = null;

process.on('message', async (msg: ReconciliationWorkerInitMessage | ReconciliationWorkerStartMessage) => {
  if (msg.type === 'INIT') {
    config = msg;
  ...
  if (msg.type === 'START' && config && persistence) {
    ...
    if (mode === 'direct') {
      ...
      try {
        ...
        const result = await inventory.reserve(requestedUnits, tokenAddress, executionId);
        if (process.send) {
          process.send({
            type: 'RESULT',
            pid: process.pid,
      ...
      try {
        const prepared = await coordinator.prepareSwap({
          idempotencyKey: 'recon_idem_' + executionId,
          hashLock,
          amountSats,
        ...
        if (process.send) {
          process.send({
            ...
            executionId,
            success: true,
            reservationId: prepared.reservationId || null,
            error: null,
            invoiceCreated,
          });
        }
